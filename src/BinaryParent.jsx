...
//                    steps:`l= ${this.state.l} r= ${this.state.r} mid= ${mid} x < ar[mid]`
    ...
    render() {
        ...
        let items = ar.map((n, index) => {return(<span id="items" key={index}>{n} <button key={index} onClick={() => {this.removeItem(index)}} type="button" className="btn-close btn-close-white" aria-label="Close"><span></span></button></span>)})
        return (
            <center>
                <div>
