...
//                steps:arr[prev.index]==prev.search && `Search element found at position: ${prev.index+1}`
    ...
    render() {
        ...
        let items = arr.map((n, index) => {return(<span id="items" key={index}>{n} <button key={index} onClick={() => {this.removeItem(index)}} type="button" className="btn-close btn-close-white" aria-label="Close"><span></span></button></span>)})
        return (
            <div>
